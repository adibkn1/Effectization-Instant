...
final class ConfigManager {
    ...
    func defaultConfiguration(for folderID: String) -> ARConfig {
        ...
        do {
            ...
            // If JSON parsing fails, return minimal fallback config
    ...
    private func defaultConfigJSON(for folderID: String) -> Data {
        ...
        let jsonString = """
        {
            "targetImageURL": "https://adagxr.com/card/\(folderID)/image.jpg",
            "videoURL": "https://adagxr.com/card/\(folderID)/video_rgb.m3u8",
            "hasTransparency": false,
            "videoRGBURL": "https://adagxr.com/card/\(folderID)/video_rgb.m3u8",
            "videoAlphaURL": "https://adagxr.com/card/\(folderID)/video_alpha.m3u8",
            "videoPlaneWidth": 1.0,
    ...
    /// Minimal fallback config: image/video URLs must always come from server config JSON, never hardcoded.
    private func createMinimalFallbackConfig() -> ARConfig {
        return ARConfig(
            targetImageURL: "", // Must be set by config JSON
            videoURL: "",       // Must be set by config JSON
            hasTransparency: false,
            videoRGBURL: "",    // Must be set by config JSON
            videoAlphaURL: "",  // Must be set by config JSON
            videoPlaneWidth: 1.0,
            videoPlaneHeight: 1.41431,
            addedWidth: 1.0,
            addedHeight: 1.0,
