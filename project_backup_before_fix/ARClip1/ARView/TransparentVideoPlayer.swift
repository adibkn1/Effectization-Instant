...
class TransparentVideoPlayer: NSObject, @unchecked Sendable {
    ...
    private func setupCombinedTexture() {
        guard let device = self.device else { 
            print("[TransparentVideo] ⚠️ Cannot setup texture - no Metal device")
            return 
        }
        
        // Ensure reasonable dimensions for the texture
        let width = max(Int(videoSize.width), 64)
        let height = max(Int(videoSize.height), 64)
        
        print("[TransparentVideo] 📐 Setting up combined texture with dimensions: \(width) x \(height)")
        ...
        if let outputTexture = outputTexture {
            ...
            print("[TransparentVideo] ✅ Created combined Metal texture: \(width)x\(height)")
        } else {
            print("[TransparentVideo] ❌ Failed to create output texture")
        }
    }
    ...
    func getMaterial() -> SCNMaterial {
        ...
        if let combinedTexture = self.combinedTexture as? MTLTexture {
            material.diffuse.contents = combinedTexture
            print("[TransparentVideo] 🎬 Using combined Metal texture for material")
        } else {
            // Otherwise just use the RGB video
            material.diffuse.contents = rgbPlayer
            print("[TransparentVideo] ⚠️ Fallback to RGB player only (no transparency)")
        }
        
        // Setup for transparency with correct blending
        material.isDoubleSided = true
        material.writesToDepthBuffer = false
        material.transparencyMode = .rgbZero  // Changed from .dualLayer to .rgbZero for better transparency
